ESX 						   = nil
local PlayersHarvestingPep     = {}
local PlayersTransformingPep   = {}
local PlayersSellingPep        = {}
...
--Pepinos
local function HarvestPep(source)

	SetTimeout(5000, function()

		if PlayersHarvestingPep[source] == true then

			local xPlayer  = ESX.GetPlayerFromId(source)

			local Pep = xPlayer.getInventoryItem('Pep')

			if Pep.limit ~= -1 and Pep.count >= Pep.limit then
				TriggerClientEvent('esx:showNotification', source, _U('inv_full_Pep'))
			else
				xPlayer.addInventoryItem('Pep', 1)
				HarvestPep(source)
			end
...
RegisterServerEvent('esx_drugs:startHarvestPep')
AddEventHandler('esx_drugs:startHarvestPep', function()

	local _source = source

	PlayersHarvestingPep[_source] = true
...
RegisterServerEvent('esx_drugs:stopHarvestPep')
AddEventHandler('esx_drugs:stopHarvestPep', function()

	local _source = source

	PlayersHarvestingPep[_source] = false

end)

local function TransformPep(source)

	SetTimeout(Config.ProcessingTime, function()

		if PlayersTransformingPep[source] == true then

			local xPlayer  = ESX.GetPlayerFromId(source)

			local PepQuantity = xPlayer.getInventoryItem('Pep').count
			local poochQuantity = xPlayer.getInventoryItem('Pep_pooch').count

			if poochQuantity > 195 then
				TriggerClientEvent('esx:showNotification', source, _U('too_many_pouches'))
			elseif PepQuantity < 5 then
				TriggerClientEvent('esx:showNotification', source, _U('not_enough_Pep'))
			else
				xPlayer.removeInventoryItem('Pep', 5)
				xPlayer.addInventoryItem('Pep_pooch', 1)
...
RegisterServerEvent('esx_drugs:startTransformPep')
AddEventHandler('esx_drugs:startTransformPep', function()

	local _source = source

	PlayersTransformingPep[_source] = true

	TriggerClientEvent('esx:showNotification', _source, _U('packing_in_prog'))

	TransformPep(_source)

end)

RegisterServerEvent('esx_drugs:stopTransformPep')
AddEventHandler('esx_drugs:stopTransformPep', function()

	local _source = source

	PlayersTransformingPep[_source] = false

end)

local function SellPep(source)


	SetTimeout(1950, function()

		if PlayersSellingPep[source] == true then
			...
			if poochQuantity == 0 then
				...
				xPlayer.removeInventoryItem('Pep_pooch', 1)
                xPlayer.addAccountMoney('black_money',1195 )
                TriggerClientEvent('esx:showNotification', source, _U('sold_one_Pep'))
				SellPep(source)
			end
...
RegisterServerEvent('esx_drugs:startSellPep')
AddEventHandler('esx_drugs:startSellPep', function()

	local _source = source

	PlayersSellingPep[_source] = true
...
RegisterServerEvent('esx_drugs:stopSellPep')
AddEventHandler('esx_drugs:stopSellPep', function()

	local _source = source

	PlayersSellingPep[_source] = false
	...
	SetTimeout(1950, function()
		...
		if PlayersSellingOpium[source] == true then
			...
			if poochQuantity == 0 then
                ...
                xPlayer.addAccountMoney('black_money', 790)
...
AddEventHandler('esx_drugs:GetUserInventory', function(currentZone)
    ...
    TriggerClientEvent('esx_drugs:ReturnInventory', 
    	_source, 
		xPlayer.getInventoryItem('Pep').count, 
		xPlayer.getInventoryItem('Pep_pooch').count,
